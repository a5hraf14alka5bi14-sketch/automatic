...
router.get('/:id', requireRole('admin','manager'), async (req, res) => {
  try {
    ...
    const orders = await pool.query(`
      ...
      WHERE o.created_at BETWEEN $1 AND COALESCE($2, NOW())
...
router.post('/open', requireRole('admin','manager'), validate(openShiftSchema), async (req, res) => {
  try {
...
router.post('/:id/close', requireRole('admin','manager'), validate(closeShiftSchema), async (req, res) => {
  ...
  try {
    ...
    try { await client.query('ROLLBACK') } catch (rbErr) {
      logger.error(`Shift close rollback failed: ${rbErr?.message}`, { path: req.path })
    }
    logger.error(`Shift close failed, transaction rolled back: ${err?.message}`, { path: req.path })
    res.status(500).json({ error: 'Failed to close shift — no changes were saved. The shift is still open; please try again.' })
