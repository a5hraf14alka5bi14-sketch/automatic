...
import { isPushConfigured } from '../integrations/push.js'
...
// Translate an OpenAIError into a user-facing status + message so the UI can
// show the real cause (e.g. exhausted quota) instead of a generic retry hint.
// Unknown errors keep the generic fallback and a 500.
function describeOpenAIFailure(err, fallbackMessage) {
  const messages = {
    no_key:              'OpenAI API key is not configured. Add a key in Integrations settings.',
    invalid_key:         'OpenAI rejected the API key. Update the key in Integrations settings.',
    quota_exceeded:      'OpenAI account quota exhausted. Add credit or check billing on platform.openai.com, then try again.',
    rate_limit:          'OpenAI rate limit hit. Wait a moment and try again.',
    service_unavailable: 'OpenAI service is temporarily unavailable. Try again shortly.',
  }
  if (err?.name === 'OpenAIError' && messages[err.code]) {
    return { status: err.status || 502, error: messages[err.code] }
  }
...
router.get('/', async (req, res) => {
  try {
    ...
    const [ghRepos, npCount, ntCount, menuCount, invCount, custCount, lastNotionSync, oaiQuotaExceeded] = await Promise.all([
      ...
      getSetting('openai_quota_exceeded')
    ])
    ...
    res.json({
      ...
      openai: {
        ...
        quota_exceeded: oaiQuotaExceeded === '1'
      },
      push: {
...
router.put('/:service/config', requireRole('admin', 'manager'), async (req, res) => {
  ...
  try {
    if (service === 'github') {
      ...
      if (apiKey?.trim()) {
        await setSetting('openai_api_key', encryptSecret(apiKey.trim()))
        await setSetting('openai_quota_exceeded', '0').catch(() => {})
      }
    } else {
...
router.post('/openai/summary', costlyIntegrationLimiter, requireRole('admin', 'manager'), async (req, res) => {
  try {
    ...
    // Build KPI snapshot directly from DB — shared helper keeps this route's
    // financial math in lockstep with /api/ai/insights (server/lib/salesKpis.js).
    const kpis = await computeSalesKpis('today')
    ...
    logger.error('[integrations/openai-summary]', { err: err?.message, code: err?.code, path: req.path })
    if (err?.code === 'quota_exceeded') await setSetting('openai_quota_exceeded', '1').catch(() => {})
    const { status, error } = describeOpenAIFailure(err, 'AI summary generation failed. Please try again.')
    res.status(status).json({ error, code: err?.code })
  }
})
...
router.post('/openai/chat', costlyIntegrationLimiter, requireRole('admin', 'manager'), async (req, res) => {
  ...
  try {
    ...
    logger.error('[integrations/openai-chat]', { err: err?.message, code: err?.code, path: req.path })
    if (err?.code === 'quota_exceeded') await setSetting('openai_quota_exceeded', '1').catch(() => {})
    const { status, error } = describeOpenAIFailure(err, 'AI request failed. Check server logs for details.')
    res.status(status).json({ error, code: err?.code })
  }
})
