...
export async function pushMenuToNotion() {
  ...
  for (const item of items.rows) {
    try {
      ...
      logger.error('[push-menu]', { id: item.id, err: e.message })
    }
  }
...
export async function pushInventoryToNotion() {
  ...
  for (const item of items.rows) {
    try {
      ...
      logger.error('[push-inventory]', { id: item.id, err: e.message })
    }
  }
...
export async function pushCustomersToNotion() {
  ...
  for (const c of customers.rows) {
    try {
      ...
      logger.error('[push-customers]', { id: c.id, err: e.message })
    }
  }
...
export async function pushRecipeIngredientsToNotion() {
  ...
  for (const item of items.rows) {
    try {
      ...
      logger.error('[push-recipe-ingredients]', { id: item.id, err: e.message })
    }
  }
  return { pushed, total: items.rows.length }
}
...
export async function pushSalesToNotion() {
  ...
  for (const order of orders.rows) {
    try {
      ...
      logger.error('[push-sales]', { id: order.id, err: e.message })
    }
  }
...
export async function pushFinanceToNotion() {
  ...
  for (const entry of entries.rows) {
    try {
      ...
      logger.error('[push-finance]', { id: entry.id, err: e.message })
    }
  }
...
export async function pushStaffToNotion() {
  ...
  for (const member of members.rows) {
    try {
      ...
      logger.error('[push-staff]', { id: member.id, err: e.message })
    }
  }
...
export async function pushMenuUpdatesToNotion() {
  ...
  for (const item of items.rows) {
    try {
      ...
      logger.error('[push-menu-update]', { id: item.id, err: e.message })
    }
  }
...
export async function pushInventoryUpdatesToNotion() {
  ...
  for (const item of items.rows) {
    try {
      ...
      logger.error('[push-inventory-update]', { id: item.id, err: e.message })
    }
  }
  return { updated, total: items.rows.length }
}
