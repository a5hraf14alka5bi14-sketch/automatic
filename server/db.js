...
export async function initDb() {
  ...
  try {
    ...
    // Safeguard: a past inventory re-seed/cleanup soft-deleted items that
    // recipe_ingredients still pointed at, silently disabling stock deduction
...
// Reactivate soft-deleted inventory items that recipe_ingredients still
// reference. Returns the number of items reactivated. Pass a client or the
// pool as `db`. Exported so tests and maintenance scripts can invoke it.
export async function reactivateRecipeLinkedInventory(db) {
  const r = await db.query(`
    UPDATE inventory SET deleted_at = NULL
    WHERE deleted_at IS NOT NULL
      AND id IN (
        SELECT DISTINCT inventory_item_id FROM recipe_ingredients
...
export async function recordStockMovement(db, {
  ...
  if (!inventoryItemId || (!change && !(allowZero && change === 0))) return
  await db.query(
