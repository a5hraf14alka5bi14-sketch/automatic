...
- Every `describe` block that seeds its own user MUST delete it in its own
  `afterAll` — the top-level cleanup only removes the users it created itself.
  A kitchen-role block once lacked this and leaked one `itest_<ts>_kitchen`
  user per run (~59 accumulated in the shared dev/prod DB before cleanup).
  When adding a seeded user to a new block, pair it with an `afterAll` delete.
