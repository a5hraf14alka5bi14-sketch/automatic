...
export default function TwoFactor({ user, onUpdated, onClose }) {
  ...
  // Disable-2FA confirmation form state
  ...
  const disableTotp = async () => {
    if (!disablePassword.trim()) return showToast('Enter your password to confirm', 'error')
    setSaving(true)
    try {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Disable 2FA — inline confirmation form requiring current password */}
      {status?.enabled && !showDisableForm && (
        <button onClick={() => setShowDisableForm(true)} disabled={saving}
      ...
      {status?.enabled && showDisableForm && (
        <div className="bg-red-500/5 border border-red-500/30 rounded-xl p-4 space-y-4">
          <div>
            <p className="text-red-400 font-semibold text-sm mb-1">Confirm to disable 2FA</p>
            <p className="text-slate-400 text-xs">Enter your current password to remove this security layer.</p>
          </div>
          <div>
            <label className="block text-slate-400 text-sm mb-2">Your current password</label>
            <input
              type="password"
          ...
          <div className="flex gap-3">
            ...
            <button onClick={disableTotp} disabled={saving || !disablePassword.trim()}
