...
export default function PaymentModal({ order, currency, onConfirm, onClose, onSplit }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/75 z-50 flex items-center justify-center p-4">
      <div ref={panelRef} role="dialog" aria-modal="true" aria-labelledby="pos-payment-title" className="bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-sm flex flex-col" style={{ maxHeight: '90vh' }}>
        <div className="p-5 border-b border-slate-800 flex-shrink-0">
          <h2 id="pos-payment-title" className="text-white font-bold text-lg">Payment</h2>
        ...
        <div className="p-5 border-t border-slate-800 flex-shrink-0 space-y-3">
          {onSplit && (
            <button onClick={onSplit}
              className="w-full py-2.5 bg-slate-800 hover:bg-slate-700 border border-slate-600 text-slate-200 rounded-xl text-sm font-medium transition-colors flex items-center justify-center gap-2">
