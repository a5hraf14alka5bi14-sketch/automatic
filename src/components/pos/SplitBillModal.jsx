...
export default function SplitBillModal({ cart, subtotal, tax, total, currency, order, onClose, onAllPaid }) {
  ...
  const payNext = async () => {
    const amount = parseFloat(customAmount) > 0 ? parseFloat(parseFloat(customAmount).toFixed(3)) : parseFloat(perPerson.toFixed(3))
        ...
        <div className="p-5 space-y-4">
          ...
          {!isComplete ? (
            <>
              ...
              <div>
                <label className="block text-slate-400 text-xs mb-1">Amount (blank = {fmtC(perPerson)})</label>
                <input type="number" step="0.001" min="0.001" value={customAmount}
                  onChange={e => setCustomAmount(e.target.value)}
                  placeholder={perPerson.toFixed(3)}
              ...
              <button onClick={payNext} disabled={paying}
                ...
                {paying ? 'Processing…' : `Collect #${payments.length + 1}`}
              </button>
            </>
          ) : (
