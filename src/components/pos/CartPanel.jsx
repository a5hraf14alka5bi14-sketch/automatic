...
export default function CartPanel({
  orderType, setOrderType, tableNum, selectTable, tablesCount,
  ...
  return (
    <div className={`${showCart ? 'flex' : 'hidden'} md:flex fixed md:static inset-0 z-20 md:z-auto w-full md:w-80 xl:w-96 border-l border-slate-800 flex-col bg-slate-950 md:bg-slate-950/30 flex-shrink-0 cart-overlay-safe`}>
      {/* Mobile: cart header with close button */}
      <div className="md:hidden flex items-center justify-between p-4 overlay-header-safe border-b border-slate-800 flex-shrink-0">
      ...
      <div className="p-4 border-b border-slate-800 flex-shrink-0">
        <div className="flex gap-1 bg-slate-900 rounded-xl p-1 mb-3">
          {[['dine-in','🍽️','Dine-in'],['takeaway','🛍️','Takeaway'],['delivery','🚚','Delivery']].map(([v,e,l]) => (
            <button key={v} onClick={() => setOrderType(v)}
              className={`flex-1 py-2 md:py-1.5 rounded-lg text-xs font-medium transition-colors flex flex-col sm:flex-row items-center justify-center gap-0.5 sm:gap-1 ${
        ...
        {/* Branch selector — only shown when more than one branch is configured */}
        {branches && branches.length > 1 && (
          <select
            ...
            onChange={e => {
              ...
              setBranchId(v)
              try { localStorage.setItem('pos_branch_id', String(v || '')) } catch {}
            }}
            className="w-full mb-3 bg-slate-800 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white focus:outline-none focus:border-orange-500"
            aria-label="Select branch"
          >
            {branches.map(b => (
              <option key={b.id} value={b.id}>{b.name}{b.name_ar ? ` · ${b.name_ar}` : ''}</option>
            ))}
          </select>
        )}
        ...
        {orderType === 'dine-in' && (
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-slate-400 text-xs">اختر الطاولة · Choose table <span className="text-red-400">*</span></span>
              {tableNum
                ? <span className="text-orange-400 text-xs font-bold">Table {tableNum}</span>
                : <span className="text-amber-400 text-xs">مطلوب · required</span>}
            </div>
            <div className="grid grid-cols-5 md:grid-cols-6 gap-1.5 max-h-32 md:max-h-28 overflow-y-auto pr-0.5">
              {Array.from({ length: tablesCount }, (_, i) => i + 1).map(n => (
                <button key={n} onClick={() => selectTable(n)}
                  aria-label={`Table ${n}`}
                  className={`h-11 md:h-8 rounded-xl md:rounded-lg text-sm md:text-xs font-bold transition-colors ${
                    tableNum === n
          ...
          <div key={item.cartId} className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
            <div className="flex items-start gap-2 p-3">
              <div className="flex-1 min-w-0">
                ...
                {item.name_ar && <p className="text-slate-400 text-xs leading-tight truncate" dir="rtl">{item.name_ar}</p>}
              ...
              <div className="flex items-center gap-2 md:gap-1.5 flex-shrink-0">
                <button onClick={() => updateQty(item.cartId, -1)} aria-label={`Remove one ${item.name}`} className="w-9 h-9 md:w-6 md:h-6 bg-slate-800 hover:bg-red-500/20 hover:text-red-400 text-slate-400 rounded-xl md:rounded-md text-base md:text-sm font-bold transition-colors flex items-center justify-center">−</button>
                <span className="text-white font-bold w-6 md:w-5 text-center text-sm" aria-live="polite">{item.qty}</span>
                <button onClick={() => updateQty(item.cartId, 1)} aria-label={`Add one ${item.name}`} className="w-9 h-9 md:w-6 md:h-6 bg-slate-800 hover:bg-green-500/20 hover:text-green-400 text-slate-400 rounded-xl md:rounded-md text-base md:text-sm font-bold transition-colors flex items-center justify-center">+</button>
              </div>
              <div className="flex flex-col items-end gap-1 flex-shrink-0">
                ...
                <div className="flex gap-1.5">
                  <button
                    ...
                    className="w-7 h-7 flex items-center justify-center text-slate-600 hover:text-slate-400 text-sm transition-colors rounded-lg hover:bg-slate-800"
                    title="Add note"
                  >📝</button>
                  <button onClick={() => removeItem(item.cartId)} className="w-7 h-7 flex items-center justify-center text-slate-600 hover:text-red-400 text-sm transition-colors rounded-lg hover:bg-red-500/10">✕</button>
                </div>
              </div>
            </div>
            {/* Item note inline */}
      ...
      <div className="p-4 border-t border-slate-800 flex-shrink-0 space-y-1.5">
        ...
        {cart.length > 0 && orderType === 'dine-in' && !tableNum && (
          <p className="text-amber-400 text-xs">⚠️ اختر رقم الطاولة أولاً · Choose a table before placing the order</p>
        )}

        {/* Place order button */}
        <button
          onClick={placeOrder}
          disabled={cart.length === 0 || placing || (orderType === 'dine-in' && !tableNum)}
          ...
          ) : orderType === 'dine-in' && !tableNum && cart.length > 0 ? (
