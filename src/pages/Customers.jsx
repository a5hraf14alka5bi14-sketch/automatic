                ...
                <div className="mt-2 pt-2 border-t border-slate-700/50 text-xs text-slate-400 space-y-0.5">
                  {o.items.slice(0, 3).map((item, i) => (
                    <div key={i} className="flex justify-between">
                      <span>{item.quantity}× {item.name}{item.name_ar ? <span dir="rtl"> · {item.name_ar}</span> : null}</span>
...
export default function Customers() {
  ...
  // Live refresh: loyalty points / total spent change when orders complete.
  const liveRefresh = useDebouncedCallback(load, 1500)
  useLiveEvents(liveRefresh, ['order_updated'])
    ...
    <div className="p-4 sm:p-6 max-w-6xl mx-auto">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => <div key={i} className="bg-slate-900 border border-slate-800 rounded-xl p-4 animate-pulse h-20" />)}
        </div>
      ) : filtered.length === 0 ? (
        ...
        <>
          ...
          <div className="md:hidden space-y-2">
            {filtered.map(c => (
              <div key={c.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 cursor-pointer active:border-orange-500/40 transition-colors"
                onClick={() => setHistoryCustomer(c)}>
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 bg-slate-700 rounded-full flex items-center justify-center text-white text-base font-bold flex-shrink-0">
                    {c.name[0].toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-white font-semibold text-sm truncate">{c.name}</p>
                    <p className="text-slate-500 text-xs truncate">{c.email || c.phone || 'No contact'}</p>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-orange-400 font-bold text-sm">{fmt(c.total_spent || 0)}</p>
                    <p className="text-slate-500 text-xs">{c.total_orders || 0} orders</p>
                  </div>
                </div>
                <div className="flex items-center gap-3 mt-2.5 pt-2.5 border-t border-slate-800">
                  {c.loyalty_points > 0 && (
                    <span className="text-xs text-orange-400 bg-orange-500/10 px-2 py-0.5 rounded-full">{c.loyalty_points} pts</span>
                  )}
                  <span className="text-slate-600 text-xs ml-auto">{new Date(c.created_at).toLocaleDateString()}</span>
                  {isManager && (
                    <div className="flex gap-2" onClick={e => e.stopPropagation()}>
                      <button onClick={() => setEditCustomer(c)} className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-orange-400 rounded-lg hover:bg-orange-500/10 transition-colors">✏️</button>
                      <button onClick={() => setDeleteCustomer(c)} className="w-8 h-8 flex items-center justify-center text-slate-500 hover:text-red-400 rounded-lg hover:bg-red-500/10 transition-colors">🗑️</button>
                    </div>
                  )}
                </div>
              </div>
