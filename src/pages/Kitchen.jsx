...
// Friendly labels/emoji for the well-known stations; any other station (added
// later, e.g. 'grill') falls back to a capitalised version of its raw value so
// it still renders sensibly in the filter without a code change.
const STATION_LABELS = { kitchen: '🍳 Kitchen', bar: '🍹 Bar', drinks: '🥤 Drinks' }
function stationLabel(s) {
  return STATION_LABELS[s] || s.charAt(0).toUpperCase() + s.slice(1)
}
              ...
              <div className="flex items-start gap-2 text-sm">
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-white ${item.done ? 'line-through' : ''}`}>{item.name}{item.name_ar ? <span className="text-slate-400" dir="rtl"> · {item.name_ar}</span> : null}</span>
                  </div>
...
export default function Kitchen() {
  ...
  // Load the managed station list from the backend so stations added, renamed
  // or retired in Settings appear here without a code change (also refetched
  // live on the 'stations_updated' WebSocket event).
  const fetchStations = useCallback(() => {
    apiFetch('/api/orders/stations')
      .then(res => res.json())
      .then(data => { if (Array.isArray(data) && data.length) setStations(data) })
      .catch(() => {})
  }, [])
  ...
  if (loading) return (
    <div className="p-3 sm:p-6">
      <h1 className="text-xl font-bold text-white mb-4">Kitchen Display</h1>
  ...
  return (
    <div className="p-3 sm:p-4 h-full flex flex-col">
      {/* Header — two rows on mobile to avoid cramping action buttons */}
      <div className="mb-3 flex-shrink-0">
        <div className="flex items-center justify-between mb-2 md:mb-0">
          <div className="min-w-0">
            <div className="flex items-center gap-2 md:gap-3">
              <h1 className="text-lg md:text-xl font-bold text-white">Kitchen<span className="hidden md:inline"> Display</span></h1>
              {rushCount > 0 && (
                <span className="bg-red-500 text-white text-xs font-bold px-2 py-0.5 rounded-full animate-pulse flex-shrink-0">
                  {rushCount} RUSH
                </span>
              )}
              <StatusDot />
          ...
          <div className="flex items-center gap-1.5 md:gap-2 flex-shrink-0">
            {/* Station filter — inline on desktop, full-width second row on mobile */}
            <div className="hidden md:flex bg-slate-800 rounded-lg p-0.5 gap-0.5">
              {[['all', 'All'], ...stations.map(s => [s, stationLabel(s)])].map(([v, l]) => (
            ...
            {/* Fullscreen — hidden on mobile since it's not useful there */}
            <button
              onClick={toggleFullscreen}
              className="hidden sm:flex w-9 h-9 items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-sm transition-colors border border-slate-700"
              title={isFullscreen ? 'Exit fullscreen' : 'Fullscreen mode'}
            >
              ⛶
            </button>
            <button onClick={fetchOrders} className="w-9 h-9 flex items-center justify-center bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-lg text-base transition-colors border border-slate-700">
              ↻
            </button>
          </div>
        </div>
        {/* Station filter — mobile only (desktop version is inline in the header) */}
        <div className="md:hidden flex bg-slate-800 rounded-lg p-0.5 gap-0.5 w-full">
          {[['all', 'All'], ...stations.map(s => [s, stationLabel(s)])].map(([v, l]) => (
            <button key={v} onClick={() => setStation(v)}
