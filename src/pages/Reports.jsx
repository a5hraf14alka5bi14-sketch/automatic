...
function Bar({ label, value, max, color = 'bg-orange-500', suffix = '' }) {
  ...
  return (
    <div className="flex items-center gap-3">
      <span className="text-slate-400 text-sm w-28 truncate">{label}</span>
                  ...
                  {data.ordersByType?.length ? (
                    <div className="space-y-3">
                      {data.ordersByType.map(row => (
                        <div key={row.type}>
                          <Bar label={row.type} value={row.count} max={data.totalOrders} suffix="" />
          ...
          {activeTab === 'profitability' && (
            <div className="space-y-6">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-6">
                ...
                <div className="space-y-3">
                  {[
                    { label: 'Gross Revenue', value: data.revenue, color: 'text-white', bar: 'bg-blue-500' },
                    { label: 'Tax Collected (11%)', value: -data.taxCollected, color: 'text-slate-400', bar: 'bg-slate-600' },
                    { label: 'Net Revenue', value: data.netRevenue, color: 'text-orange-400 font-bold', bar: 'bg-orange-500', sep: true },
                    { label: 'Food Cost', value: -data.totalFoodCost, color: 'text-red-400', bar: 'bg-red-500' },
                ...
                <div className="mt-5 pt-4 border-t border-slate-800">
                  ...
                  <div className="w-full bg-slate-800 rounded-full h-3">
                    <div
                      className={`h-3 rounded-full transition-all ${data.grossMargin >= 65 ? 'bg-green-500' : data.grossMargin >= 45 ? 'bg-yellow-500' : 'bg-red-500'}`}
              ...
              {data.categoryPerf?.length > 0 && (
                <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                  <div className="px-5 py-4 border-b border-slate-800">
                    <h2 className="text-white font-semibold">Category Profitability</h2>
                  </div>
                          ...
                          <tr key={cat.category} className="border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors">
                            <td className="py-3 px-4">
                              <span className="text-white text-sm capitalize">
                                {CAT_EMOJI[cat.category] || '🍽️'} {cat.category}
                              </span>
                            </td>
                            ...
                            <td className="py-3 px-4 text-right">
                              <span className={`text-sm font-medium ${profit >= 0 ? 'text-green-400' : 'text-red-400'}`}>{fmt(profit)}</span>
                            </td>
                            <td className="py-3 px-4 text-right">
                              <span className={`text-sm font-bold ${marginColor(cat.marginPct)}`}>{cat.marginPct}%</span>
                            </td>
                      ...
                      <tbody>
                        {data.lowStock.map((item, i) => {
                          const pct = parseFloat(item.min_quantity) > 0
                            ? Math.round((parseFloat(item.quantity) / parseFloat(item.min_quantity)) * 100)
                            : 0
                          ...
                          return (
                            <tr key={i} className="border-b border-slate-800/50 hover:bg-slate-800/30">
                              ...
                              <td className="py-3 px-4 text-right">
                                <span className={`font-semibold text-sm ${isEmpty ? 'text-red-400' : 'text-yellow-400'}`}>
