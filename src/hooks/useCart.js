...
export function useCart({ taxRate = 0, stockAvail = {}, showToast } = {}) {
  ...
  const addToCart = (item, selectedModifiers = []) => {
    ...
    setCart(prev => {
      ...
      return [...prev, { cartId, id: item.id, name: item.name, name_ar: item.name_ar || null, price: unitPrice, qty: 1, modifiers: selectedModifiers, category: item.category, station: item.station || null }]
    })
  }
